...
function AreaEstudiante({ estudiante, progreso, grados, onClose, onAbrir }) {
  ...
  const COLOR_CATEGORIA = {
    "🎓 Académico y seguimiento": "#7C3AED",
    "🤝 Convivencia": "#F59E0B",
    "🧩 Apoyo e inclusión": "#3B82F6",
    "🪪 Datos y administración": "#64748B",
    "🎮 Vida en CÓDICE": "#10B981",
  };
    ...
    <div className="fixed inset-0 z-20 bg-white overflow-y-auto">
      <div className="max-w-2xl mx-auto p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {CATEGORIAS.map((cat) => {
            const color = COLOR_CATEGORIA[cat.titulo] || "#7C3AED";
            return (
              <div key={cat.titulo} className="rounded-2xl border p-4" style={{ background: `${color}0D`, borderColor: `${color}40` }}>
                <h3 className="text-xs font-bold uppercase tracking-wide mb-3" style={{ color }}>{cat.titulo}</h3>
                <div className="space-y-2">
                  {cat.items.map((item) => (
