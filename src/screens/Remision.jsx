...
function CampoTexto({ label, value, onChange, tipo = "text", filas }) {
  return (
    <div className="mb-2">
...
function RemisionForm({ estudiante, remision, onCancelar, onGuardado }) {
  ...
  return (
    <div className="bg-violet-50 rounded-2xl p-4 mb-4">
      ...
      <CampoTexto label="1. Descripción de la situación observada" filas={3} value={f.descripcion_situacion} onChange={set("descripcion_situacion")} />
      ...
      <CampoTexto label="Detalle (tiempos, espacios, metodología, didáctica)" filas={2} value={f.estrategias_descripcion} onChange={set("estrategias_descripcion")} />
      ...
      <CampoTexto label="Compromisos acordados" filas={2} value={f.compromisos_acordados} onChange={set("compromisos_acordados")} />
      ...
      <CampoTexto label="Motivo de esa remisión" filas={2} value={f.motivo_remision_otras} onChange={set("motivo_remision_otras")} />
      <CampoTexto label="Acciones realizadas" filas={2} value={f.acciones_realizadas} onChange={set("acciones_realizadas")} />
...
function RemisionPrintView({ remision, estudiante, institucion, onCerrado }) {
  ...
  const contenido = (
    <div className="print-only" style={{ maxWidth: 800, margin: "0 auto", padding: 24, fontFamily: "Arial, sans-serif", fontSize: 11, color: "#111" }}>
      ...
      <div style={{ border: "1px solid #000", minHeight: 50, padding: 6, marginBottom: 10, whiteSpace: "pre-line" }}>{remision.descripcion_situacion}</div>
      ...
      <div style={{ border: "1px solid #000", minHeight: 40, padding: 6, marginBottom: 10, whiteSpace: "pre-line" }}>{remision.estrategias_descripcion}</div>
      ...
      <div style={{ border: "1px solid #000", minHeight: 30, padding: 6, marginBottom: 10, whiteSpace: "pre-line" }}>{remision.compromisos_acordados}</div>
      ...
      <div style={{ marginBottom: 4 }}>Motivo de la remisión: {remision.motivo_remision_otras}</div>
      <div style={{ marginBottom: 16 }}>Acciones realizadas: {remision.acciones_realizadas}</div>
            ...
            {remisiones.map((r) => (
              <div key={r.id} className="bg-slate-50 rounded-xl p-3 flex justify-between items-start gap-2">
                <div>
                  ...
                  {r.descripcion_situacion && <div className="text-xs text-slate-400 mt-1 line-clamp-2">{r.descripcion_situacion}</div>}
                </div>
