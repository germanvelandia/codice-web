...
export async function marcarAsistencia(estudianteId, fecha, codigo, observacion, materiaId = null) {
  ...
  const { error } = await supabase.from("asistencia").upsert(
    { estudiante_id: estudianteId, fecha, codigo, observacion: observacion || null, materia_id: materiaId, docente_id: userData?.user?.id || null, xp_aplicado: efecto.xp, vida_aplicada: efecto.vida },
